...
class FastDDSProfile(Substitution):
    ...
    def __init__(
        ...
        self.external_interfaces = (
            [ResolveHost(iface) for iface in external_interfaces] if external_interfaces else []
        )
    ...
    def perform(self, context: LaunchContext) -> str:
        ...
        self.template_vars["external_discovery_servers"] = (
            [
                resolve_endpoint(srv, context)
                for srv in self.template_vars["external_discovery_servers"]
            ]
            if self.template_vars["external_discovery_servers"]
            else []
        )
        self.template_vars["local_discovery_server"] = (
            resolve_endpoint(self.template_vars["local_discovery_server"], context)
            if self.template_vars["local_discovery_server"]
            else None
        )
